...
const send = (res, messages, outputContexts) =>
  res.json(outputContexts ? { fulfillmentMessages: messages, outputContexts } : { fulfillmentMessages: messages });
...
const buildSearchWhere = ({ product_name, capacity, color }) => {
  ...
  if (color) {
    whereVariant[Op.and] = [
      ...(whereVariant.variant_name ? [{ variant_name: whereVariant.variant_name }] : []),
...
async function handleSearchProduct(q, res) {
  const session      = q.session || q.outputContexts?.[0]?.name?.split("/contexts/")[0];
  const product_name = param(q, "product_name");
  const brand        = param(q, "brand");
  const capacity     = param(q, "capacity");
  const color        = param(q, "color");

  const { whereProduct, whereVariant } = buildSearchWhere({ product_name, capacity, color });
  ...
  const include = [
    ...
    },
    {
      ...
      as: VARIANT_AS,
      attributes: ["variant_id", "variant_name", "price", "stock_quantity", "image_url", "variant_sku", "item_status"],
  ...
  if (!products.length) {
    return send(res, [dfText("Chưa tìm thấy sản phẩm phù hợp. Bạn mô tả rõ hơn giúp mình nhé?")]);
  }
  ...
  if (!rows.length) {
    return send(res, [dfText("Hiện chưa có biến thể khả dụng cho các sản phẩm tìm thấy.")]);
  }

  const topN = rows.length;
  const summary = rows.map((r, i) => `${i + 1}. ${r.name} — ${r.price} — đã bán ${r.sold}`).join("\n");

  return send(
    res,
    [
      // ✅ Chuỗi text để demo trực tiếp trong Dialogflow Console
      dfText(`Chúng tôi có một số mẫu để Anh/Chị tham khảo (Top ${topN}):\n${summary}`),
    ...
    session ? [buildCtx(session, "ctx_product", 5, { product_name, brand, capacity, color })] : undefined
  );
}
...
async function handleProductPrice(q, res) {
  ...
  if (!products.length) {
    return send(res, [dfText("Chưa thấy mức giá phù hợp. Bạn mô tả rõ hơn (model/brand/dung lượng/màu) giúp mình nhé?")]);
  }
  ...
  if (!rows.length) {
    return send(res, [dfText("Hiện chưa có biến thể định giá cho các sản phẩm này.")]);
  }
...
async function handleProductStock(q, res) {
  ...
  if (!products.length) {
    return send(res, [dfText("Mình chưa thấy tồn kho cho yêu cầu này. Bạn cho mình model chính xác hơn nhé?")]);
  }
  ...
  if (!rows.length) {
    return send(res, [dfText("Hiện chưa có biến thể còn hàng cho các sản phẩm này.")]);
  }

  const topN = rows.length;
  const summary = rows.map((r, i) => `${i + 1}. ${r.name} — ${r.price} — tồn ${r.stock}`).join("\n");

  return send(
    res,
    [
      dfText(`Các lựa chọn còn hàng (Top ${topN}):\n${summary}`),
      dfPayload({ type: "product_list", title: "Các lựa chọn còn hàng:", items: rows }),
    ]
  );
}
...
async function handleOrderStatus(q, res) {
  ...
  if (!orderRow) return send(res, [dfText(`Chưa thấy đơn #${id}. Bạn kiểm tra lại giúp mình nhé!`)]);
  ...
  const text = `Đơn #${orderRow.order_id} đang ở trạng thái *${orderRow.order_status}*, thanh toán: ${orderRow.payment_status}, tổng ${money(orderRow.order_total)}. Tạo lúc ${new Date(orderRow.created_at).toLocaleString("vi-VN")}.`;
...
const handleWebhook = async (req, res) => {
  try {
    ...
    switch (intent) {
      case "SearchProduct":   return await handleSearchProduct({ ...q, session }, res);
      case "ProductPrice":    return await handleProductPrice({ ...q, session }, res);
      case "ProductStock":    return await handleProductStock({ ...q, session }, res);
      case "OrderStatus":     return await handleOrderStatus({ ...q, session }, res);
      case "HandoverToAgent": return await handleHandover(q, res);
      default:                return send(res, [dfText("Bạn mô tả rõ hơn giúp mình với ạ?")]);
    }
  } catch (err) {
    console.error(err);
